...
class DiagnosticsTest(unittest.TestCase):
    def test_requests_diagnostics_for_a_file(self):
        ...
        self.assertEqual(c.calls[-1]["method"], "textDocument/diagnostic")
...
def diagnostics(window, file_path: Optional[str] = None, client=None) -> Dict[str, Any]:
    ...
    if file_path:
        ...
        if not view:
            return {"error": "No active view"}
    session, err = c.session_for_view(view)
    if err:
        return {"error": err}
    result, err = c.request(session, "textDocument/diagnostic", c.document_params(view), view)
    if err:
        return {"error": err}
    items = (result or {}).get("items", []) if isinstance(result, dict) else (result or [])
    out = [{
...
Then delete `mcp_server.py:1169-1540` — the whole `# ─── LSP Tools ───` block including `_resolve_file_view`, `_get_lsp_session`, `_lsp_request_sync`, the six `_lsp_*` methods, `_uri_to_path`, `_parse_locations`, `_flatten_document_symbols`, `_symbol_kind_name`.
...
- `python3.14 -m pytest -q` reports 433 passed
